...
let update = () => {
    ...
    // task_start ※必須項目
    let task_start = document.getElementsByName('task_start')[0].value
    form.task_start = task_start.indexOf(',') == -1 ? task_start : task_start.replace(',', '')

    // task_end ※必須項目ではない
    let task_end = document.getElementsByName('task_end')[0].value

    // 開始時間　＜　終了時間　のチェック
    // true: であればエラー出す / false: form.task_endに値を代入
    if(task_end != '') {
        if(new Date(form.task_start) >= new Date(task_end)) {
            taskTimeError.flg = true
            return;
        } else {
            taskTimeError.flg = false

            // 「,」 の取り除いた後代入
            form.task_end = task_end.indexOf(',') == -1 ? task_end : task_end.replace(',', '')
        }
    }

    // available start
    form.available_start = document.getElementsByName('available_start_time')[0].value

    // available end
    form.available_end = document.getElementsByName('available_end_time')[0].value
                ...
                <header class="text-gray-600 body-font">
                    <div class="container mx-auto flex flex-wrap flex-col md:flex-row items-center">
                        ...
                        <nav class="md:ml-auto flex flex-wrap items-center text-base justify-center">
                            ...
                            <div v-else><Link :href="route('horenso', { id: props.user[0].id })">報連相する</Link></div>
                        </nav>
                    </div>
                </header>
            </form>
                    ...
                    <div class="p-6 text-gray-900">
                        ...
                        <section class="text-gray-600 body-font">
                            <div class="container px-5 py-3 mx-auto flex flex-wrap flex-col">
                                ...
                                <!-- タブの中身 -->
                                <KeepAlive>
                                    <Task v-show="activeNum == 1" :form="form" :errors="props.errors" :taskTimeError="taskTimeError.flg"></Task>
                                </KeepAlive>
                                <KeepAlive>
                                    <Schedule v-show="activeNum == 2" :form="form" :errors="props.errors"></Schedule>
                                </KeepAlive>
                                <KeepAlive>
                                    <Available v-show="activeNum == 3" :form="form" :errors="props.errors"></Available>
                                </KeepAlive>
                                <KeepAlive>
                                    <Strong v-show="activeNum == 4" :form="form" :errors="props.errors"></Strong>
                                </KeepAlive>
                                <KeepAlive>
                                    <Record v-show="activeNum == 5" :rec="props.rec" :errors="props.errors"></Record>
                                </KeepAlive>
                                <!-- タブの中身 -->
